...
const MOCK_STREAMED_RESPONSE_BODY = `data: {"event": "start", "data": {"conversation_id": "${CONVERSATION_ID}"}}
...
const MOCK_STREAMED_RESPONSE_WITH_ERROR_BODY = `data: {"event": "start", "data": {"conversation_id": "${CONVERSATION_ID}"}}
...
const MOCK_MCP_STREAMED_RESPONSE_BODY_TEMPLATE = `data: {"event": "start", "data": {"conversation_id": "CONVERSATION_ID"}}
...
Cypress.Commands.add(
  ...
  (
    ...
    cy.intercept('POST', getApiUrl('/v1/streaming_query'), (request) => {
      ...
      request.reply({ body: responseBody, delay: 1000 });
    }).as(alias);
  },
);
...
Cypress.Commands.add(
  ...
  (
    ...
    cy.intercept('POST', getApiUrl('/v1/mcp-apps/resources'), (request) => {
      /* eslint-disable camelcase */
      if (request.body.resource_uri !== uiResourceUri) {
        // Not the request we're waiting for — let it fall through to older intercepts
        return;
      }
